...
export class OrderService {
  ...
  async create(
    ...
    this.httpClientService.post({ controller: 'orders' }, order).subscribe(
      (result) => {},
  ...
  read(): Observable<ListOrder[]> {
    var a = this.httpClientService.get<ListOrder[]>({ controller: 'orders' });
    return a;
  ...
  async readStatusDaysCount(
    FilterDays: number,
    StatusType: string
  ): Promise<Observable<DatetableOrder[]>> {
