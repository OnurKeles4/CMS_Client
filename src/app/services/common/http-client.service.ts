...
export class HttpClientService {
  ...
  getDays<T>(
    ...
    if (requestParameter.fullEndPoint) url = requestParameter.fullEndPoint;
      ...
      url = `${this.url(requestParameter)}${`/last-month?value=${FilterDays}`}${
        id ? `/${id}` : ''
      }${
        requestParameter.queryString ? `?${requestParameter.queryString}` : ''
      }`;
  ...
  getDaysCount<T>(
    ...
    if (requestParameter.fullEndPoint) url = requestParameter.fullEndPoint;
      ...
      url = `${this.url(
        requestParameter
      )}${`/count-orders?value=${FilterDays}`}${id ? `/${id}` : ''}${
        requestParameter.queryString ? `?${requestParameter.queryString}` : ''
      }`;
    return this.httpClient.get<T>(url, { headers: requestParameter.headers });
  }

  getStatusDaysCount<T>(
    requestParameter: Partial<RequestParameters>,
    FilterDays?: number,
    StatusType?: string
  ): Observable<T> {
    ...
    if (requestParameter.fullEndPoint) url = requestParameter.fullEndPoint;
      ...
      url = `${this.url(
        requestParameter
      )}${`/count-status-orders?value=${FilterDays}`}${`&status=`}${
